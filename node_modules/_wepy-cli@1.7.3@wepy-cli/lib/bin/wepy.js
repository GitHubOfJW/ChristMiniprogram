...
_commander2.default.command('devtool <action-name> [path]').description('commander for login, preview, upload and other operations.').action(require('./wepy-devtool')).option('--login-qr-output [format[@path]]', '指定二维码输出形式.').option('--preview-qr-output [format[@path]]', '指定二维码输出形式，语义同登录用的选项 --login-qr-output.').option('--upload-desc <desc>', '上传代码时的备注.').option('-q, --quiet', '不提供参数引导').on('--help', function () {
    console.log();
    console.log('  Example:');
    console.log();
    console.log(_chalk2.default.gray('   # 启动登录逻辑，将二维码打印在命令行中。'));
